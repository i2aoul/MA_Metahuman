...
// Sets default values
AMeshCoordsCharacter::AMeshCoordsCharacter()
{
    PrimaryActorTick.bCanEverTick = true;
    // Allgemeine Bewegungseinstellungen
    ...
    MovementComponent->DefaultWaterMovementMode = MOVE_Swimming;
    ...
    // Sprung-/Fallparameter
    MovementComponent->JumpZVelocity = 500.0f;
    MovementComponent->AirControl = 0.25f;

    // Rotation
    MovementComponent->bOrientRotationToMovement = true; // Charakter richtet sich nach der Bewegung aus
    MovementComponent->RotationRate = FRotator(0.f, 540.f, 0.f);

}

// Called when the game starts or when spawned
void AMeshCoordsCharacter::BeginPlay()
{
    Super::BeginPlay();
...
// Called every frame
void AMeshCoordsCharacter::Tick(float DeltaTime)
{
    Super::Tick(DeltaTime);
    // FindAllCharactersAndTheirComponents();
}

void AMeshCoordsCharacter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AMeshCoordsCharacter::ProcessBones(USkeletalMeshComponent* InMeshComponent, UCameraComponent* InCameraComponent, const TArray<FString>& FilterKeywords, TMap<int32, FString>& BoneCache)
{
...
void AMeshCoordsCharacter::DrawBoneDebug(const FVector& WorldPosition, int32 BoneIndex, const FName& BoneName)
{
    DrawDebugSphere(
        GetWorld(),
        WorldPosition,
...
bool AMeshCoordsCharacter::BoneNameMatchesFilter(const FName& BoneName, const TArray<FString>& FilterKeywords)
{
...
void AMeshCoordsCharacter::FindAllCharactersInLevel()
{
...
void AMeshCoordsCharacter::FindAllCharactersAndTheirComponents()
{
    TArray<AActor*> Characters;
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), ACharacter::StaticClass(), Characters);
...
void AMeshCoordsCharacter::SaveCachedDataToCSV(const FString& FilePath, TMap<int32, FString>& BoneCache)
{
...
void AMeshCoordsCharacter::DrawDebugIterationsFromCSV(const FString& FilePath, int32 IterationIndex)
{
...
// Press Ctrl key
void AMeshCoordsCharacter::PressCtrlKey()
{
#if PLATFORM_WINDOWS
...
// Release Ctrl key
void AMeshCoordsCharacter::ReleaseCtrlKey()
{
#if PLATFORM_WINDOWS
...
// Press Shift key
void AMeshCoordsCharacter::PressShiftKey()
{
#if PLATFORM_WINDOWS
...
// Release Shift key
void AMeshCoordsCharacter::ReleaseShiftKey()
{
#if PLATFORM_WINDOWS
...
void AMeshCoordsCharacter::MouseLeftButtonDown()
{
#if PLATFORM_WINDOWS
...
void AMeshCoordsCharacter::MouseLeftButtonUp()
{
#if PLATFORM_WINDOWS
...
// Space key down
void AMeshCoordsCharacter::SpaceKeyDown()
{
#if PLATFORM_WINDOWS
    // Simuliere das Drücken der Leertaste
    keybd_event(VK_SPACE, 0, 0, 0);
    UE_LOG(LogTemp, Warning, TEXT("Space key down"));
#endif
}
